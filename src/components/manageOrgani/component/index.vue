    ...
    <div class="content-box" v-loading = "loading2">
      <div v-if="!lookDetail">
        ...
        <el-dialog title="批量导入" :visible.sync="batchInDialogTableVisible">
          ...
          <div v-if="saveText"><i class="iconfont icon-zhongguohangtiantubiaoheji-weizhuanlunkuo-" style="text-align: left;padding-left: 40px; color: #942987;font-size: 25px"></i>已保存</div>
          <div style="text-align:center;width:100%;height:40px;margin-right:20px;margin-bottom: 20px">
          ...
          <el-button type="primary" ><a style="color:#fff;text-decoration: none;" href="http://www.2nankai.cn/api/Execl/exportTemplate?type=0">下载教师模板</a></el-button>
          <el-button type="primary" ><a style="color:#fff;text-decoration: none;" href="http://www.2nankai.cn/api/Execl/exportTemplate?type=1">下载学生模板</a></el-button>
          <el-button type="primary" ><a style="color:#fff;text-decoration: none;" href="http://www.2nankai.cn/api/Execl/exportTemplate?type=2">下载家长模板</a></el-button>
        </el-dialog>
        <!--批量导出弹层-->
        <el-dialog title="批量导出" :visible.sync="batchOutDialogTableVisible">
          ...
          <div v-if="step===0">
            ...
            <div>
              <el-select v-model="excelUserType" style="text-align: left;width: 160px; float: left;" placeholder="请选择用户类型">
                <el-option
                  v-for="item in excellArr"
                  :key="item.excelUserType"
                  :label="item.excelUserTypeLab"
                  :value="item.excelUserType">
                </el-option>
              </el-select>
            </div>
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <span v-if="scope.row.locked == '停用'" style="color: #85257a">停用</span>
              ...
              <el-form :model="addTableList" style="display: flex;flex-wrap: wrap"  :rules="rules" ref="addTableList" label-width="100px">
               <!-- <el-form-item label="姓名"  style="flex: auto;width: 30%;">-->
                <div style="margin-bottom: 10px;text-align: left;width:100%;">
                  <el-select v-model="school.schoolKey" style="width:40%"  placeholder="二南开学校" @change="userSchoolChange(school.schoolKey)">
                    <el-option
                ...
                <div style="margin-bottom: 10px;">
                  <el-input  v-model="addTableList.studentNum" v-if="addTableList.userType!='0' " @blur="changeRegisterNumber(addTableList.studentNum)"  style="width:180px;margin-right: 25px;float: left" placeholder="请输入学生编号"></el-input>
                  <el-input  v-model="addTableList.usermark" v-if="addTableList.userType!='0' " @blur="changeUsermarkNumber(addTableList.usermark)"  style="width:180px;margin-right: 25px;float: left" placeholder="请输入学籍号"></el-input>
                  <el-select v-model="userParentVal" v-if="addTableList.userType!='0'&&addTableList.userType=='2'" @change="selectUserParent(userParentVal)" style="float: left;display: block" placeholder="请选择家属关系">
                    <el-option
                      v-for="item in userParent"
                      :key="item.id"
                      :label="item.appellation"
                      :value="item.id">
                    </el-option>
                  </el-select>
                </div>
          ...
          <el-container>
            ...
            <el-main class="showDetailMain">
              ...
              <ul style="text-align: left;line-height: 30px">
                ...
                <li>所在学校 : {{userDetalData.schoolid}}</li>
                <li v-if="userDetalData.usertype_int ===0">区账号 : {{userDetalData.areaAccount}}</li>
                <li v-if="userDetalData.usertype_int !==0">学籍号 : {{userDetalData.usermark}}</li>
                <li v-if="userDetalData.usertype_int !==0">国学籍号 : {{userDetalData.countryUserMark}}</li>
              </ul>
            </el-main>
          </el-container>
          <div> <!--第三方绑定-->
...
//        console.log(`每页 ${val} 条`);
        ...
        hanleAdd(){
          ...
          API.getUserTypeSitua().then((res)=>{
            ...
            APILOG.appellationList({}).then((res)=>{//家长称呼列表
				console.log(res)
        ...
        userSchoolChange(val){
      	  console.log(val);
          this.schoolChooseName = val;
          this.userIsExistChange(this.addTableList.realname);
          if(this.addTableList.userType === 1 ||this.addTableList.userType === 2 ){
            this.changeRegisterNumber(this.addTableList.studentNum);
          }
        },
        //选择用户情况
        userSituations(val){
        ...
        userIsExistChange(userName){
            ...
            if(this.addTableList.userType === 0){//教师
                if(this.schoolChooseName === 1){
                  this.addTableList.username ='t'+ p;
                }else if(this.schoolChooseName === 2){
                this.addTableList.username ='txn'+ p;
                }
          ...
          val = val.replace(/\D/g,"");
            ...
            if(this.addTableList.userType === 1){//学生
              if(this.schoolChooseName === 1){
                ...
                this.addTableList.username ='xxn'+ p;
              }
...
//      	console.log(this.jzAccount)
		  			...
		  			console.log(res)
			            if(res.code==1){
			            	if(res.data.exist===0){//不存在此关系，可以添加
						        ...
						        this.randomnum = rand;
								this.addTableList.password = this.addTableList.username + this.randomnum;
			            	}else{
			            		this.$message({
					                title: '操作提示',
        ...
        selectUserParent(val){
          console.log(val)
          let obj = {};
		  obj = this.userParent.find((item)=>{
		        return item.id === val;//比如：选项2
		  });
		  this.addTableList.rel = obj.appellation;
        ...
        //添加提交
        submitForm(value){
      	  if(this.schoolChooseName == ''){
            this.$notify.error({
              title: '错误',
              message: '请选择学校！'
            });
          }else if(this.rePhone == 'true'){
            this.$notify.error({
              title: '错误',
            ...
            if(this.reEmail == 'true'){
              ...
              if(value.userType === ''){
                ...
                let promise ={
                  ...
                  pwd: this.addTableList.password,
        ...
        lookUserDetail(index,value){
          ...
          this.userSeltImg = value.adminimg;
          this.usertypeimg = value.currimg;
          if(value.schoolid == '1'){
            value.schoolid = '二南开学校'
          }else if(value.schoolid == '2'){
            value.schoolid = '兴南中学'
          }
        ...
        nextStep3(){
            ...
            let sectionid = sectionIds;
			      ...
			      if(this.excelUserType == '2'){
              window.location.href =  API.dlmodel + '/Execl/exportPatriarch?secId=' + sectionid;
            }else {
              window.location.href = API.dlmodel + '/Execl/export?secId=' + sectionid + '&userType=' + this.excelUserType;
            }
          ...
          if(this.multipleSelection.length==0){
            ...
            /*  this.multipleSelection.forEach( item =>{
                if(item.section && item.section.length>0){
                  console.log(item.section + "sectionid");
                  console.log(item.index)
                  console.log(item.user_primary_id + 'user_primary_id');
                  let index = item.section.findIndex(d => d.sectionid === item.user_primary_id);
                  console.log(index);
                  if(index === -1){
                    flag = 'false'
                  }else {
              ...
              let id = this.multipleSelection[0].user_primary_id;
              console.log(id);
              this.multipleSelection.forEach(item=>{
                console.log(item.section)
                let index = item.section.findIndex(d => d.sectionid === id);
                console.log(index);
                if(index === -1){
                  return  flag = 'false'
                }else {
            ...
            if(flag !== 'false'){
              ...
              this.$notify({
                title: '操作提示',
                message: '用户不同部门时不可以批量设置部门,请单个设置',
                iconClass: 'el-icon-info',
                offset: 70
              });
            }
