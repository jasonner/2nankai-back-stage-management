        ...
        <div class="box-content">
            <div class="role" style="border-bottom:1px solid #F6F6F6;padding-bottom:20px;margin-bottom:20px;">
	            ...
	            <el-form style="margin-bottom:10px;border-bottom:1px dashed #eee">          
	                ...
	                <el-form-item style="display:inline-block;">
	                    <el-select :loading="loadingClass" v-model="className" @change="classChange(className)" placeholder="班级选择">
	                    	<el-option
	                ...
	                <!--<tableList :checkBox="true" :tableData='subjectAllUser' :pageNum="params.page" :pageSize="params.pageSize" :total="total" :columns="columns" @CurrentChange="handleCurrentChange" @selectionChange="handSelectionChange"></tableList>-->
      				<el-table
					    ...
					    <el-table-column
					      ...
					      label="主要部门"
					      show-overflow-tooltip>
					      <!--<template slot-scope="scope"> {{scope.user_primary}}</template>-->
					    </el-table-column>
...
//        	render:(h,param) =>{
//        		console.log(param.row.user_primary.name)
//              return h('span', param.row.user_primary.name)
//          }
//        },
...
//			})
	    ...
	    cancel(){
	    	this.$router.push({path:'/lecture', query: {show: true}})
	    },
	    submit() {
	    	if(this.subjectName&&this.gradeName&&this.className){
	    		let allArr=[];
	    		...
	    		if(allArr.length>0){	
		    		...
		    		let promise={
		    			...
		    			skteachers : userIds,//用户id
		    			gradeId : this.gradeName,//年级id
		    			classNum:this.className//班级id
		    		}
		    		API.skLeaderCreate(promise).then((res)=>{
		    			console.log(res);
		    			if(res.code==1){
		    				this.$message({
				                title: '操作提示',
