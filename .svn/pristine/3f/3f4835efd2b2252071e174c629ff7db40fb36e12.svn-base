 ...
 <template>
    <div class="login-wrap">
        ...
        <div class="login-wrap-center">
            <img src="../../../static/logoOf2nankai.png" style="position:relative;height:75px;display:block;padding-top:3%;margin:0 auto;">
            <div class="ms-box">
            ...
            <div class="download">
                        ...
                        <p>点击下载最新浏览器</p>
                        <ul>
                            <li  v-for="item in liulanqi"><a :href="item.browserurl">
                                <img :src="item.img" alt="" style="width:54px;height:54px;">
                                <p>{{item.browsername}}</p>
                            </a></li>
...
//	                }
      ...
      mounted(){
      	...
      	APILOG.BrowserList({}).then((res)=>{//浏览器列表
        ...
        if(this.$route.query.code !=''&&this.$route.query.code !==undefined){
          let promise = {
            ...
            APILOG.wexinLogin(promise).then((res)=>{
            	console.log(res)
            if(res.code===1){
...
//        	this.passwordchange == "password"?"text":"password";
            ...
            wechatLogin(){
              window.location.href = 'https://open.weixin.qq.com/connect/qrconnect?appid=wxfc49324f37e84eca&redirect_uri=http%3A%2F%2Fwww.2nankai.cn%2Fcer%2F%23%2Flogin&response_type=code&scope=snsapi_login&state=STATE#wechat_redirect';
              // API.signIn().then((res)=>{
              //   self_redirect:true,
              //   id:"wechat",
            ...
            submitForm(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
						let result = this.ruleForm.register.replace(/^(\s|\u00A0)+|(\s|\u00A0)+$/g,'');
                    	let loginInfo = {
                    		register: result,
                    	...
                    	API.loginyz(loginInfo).then((res)=>{
                    		...
                    		if(res.code==1){
                    			...
                    			if(res.data.count>1&&res.data.count<res.data.limit){
                    				this.$notify({
	                                  title: '操作提示',
	                                  message: '您已经连续'+res.data.count+'次密码错误，连续'+res.data.limit+'次账号将被冻结30分钟！',
	                                  iconClass: 'el-icon-info',
	                                  offset: 70
	                                });
                    				...
                    				if(res.data.time==''){
                    					this.$notify({
		                                  title: '操作提示',
		                                  message: '您已经连续'+res.data.limit+'次密码错误，账户已冻结，请30分钟后重试',
		                                  iconClass: 'el-icon-info',
		                                  offset: 70
		                                });
                    				}else{
                    			...
                    			this.$router.push({  path:'stop',query:{time:res.data.argumentsvalue1}  });
...
<style scoped>
  ...
  .ms-third .third-tools{width:100%;padding:0;}
  .ms-third .third-tools li{float:left;width:100%;margin:0 auto;text-align:center;cursor:pointer;}
